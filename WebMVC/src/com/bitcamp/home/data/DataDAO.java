...
public class DataDAO extends DBCPconn implements DataDAOImpl {
	...
	public void dataSelect(DataVO vo) {
		try {
			getConn();
			sql = "select no, title, content, userid, hit, downcount, writedate, filename1, filename2 from data where no=?";
			pstmt = con.prepareStatement(sql);
			...
			System.out.println("레코드 선택DAO 에러 ~~~");
			e.printStackTrace();
		}finally {
			getClose();
		}
	...
	public int dataUpdate(DataVO vo, List<String> newFile) {
		...
		try {
			...
			System.out.println("자료실 글수정 DAO 에러~~");
			e.printStackTrace();
		}finally {
			getClose();
		}
	...
	public int dataDelete(DataVO vo) {
		...
		try {
			...
			System.out.println("레코드 삭제 DAO 에러!!");
			e.printStackTrace();
		}finally {
			getClose();
		}
	...
	public List<DataVO> dataSelectAll() {
		...
		try {
			...
			System.out.println("자료실 전체선택 에러");
			e.printStackTrace();
		}finally {
			getClose();
		}
	...
	public int downloadCount(int no) {
		...
		try {
			...
			System.out.println("다운로드 횟수 증가 dao에러~~");
			e.printStackTrace();
		}finally {
			getClose();
		}
	...
	public void filenameSelect(DataVO vo) {
		try {
			...
			System.out.println("파일명 선택 DAO 에러!!");
			e.printStackTrace();
		}finally {
			getClose();
		}
	...
	public List<String> getSelectFile(int no){
		...
		try {
			...
			System.out.println("파일선택에러...getselectFile()----");
			e.printStackTrace();
		}finally {
			getClose();
		}
