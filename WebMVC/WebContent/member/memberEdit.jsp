...
<div class="container">
	...
	<form method="post" id="memId" action="<%=request.getContextPath() %>/member/memberEditOk.do">
		<ul id="regForm">
			...
			<li>관심분야</li>
			<li>	<!-- for문으로 배열처리를 하고 그 배열의 i번째를 체크하여 값이 있으면 checked처리 -->
				...
				<input type="checkbox" name="interest" value="게임" <c:forEach var="i" items="${vo.interest}"><c:if test="${i=='게임'}">checked</c:if></c:forEach>/>게임
				<input type="checkbox" name="interest" value="웹툰" <c:forEach var="i" items="${vo.interest}"><c:if test="${i=='웹툰'}">checked</c:if></c:forEach>/>웹툰
			</li>
			<li>
